...
task("generate-deployer", async (args, hre) => {
    ...
    console.log(
        "Take this 12 word phrase in a secure place and enter a password!\n"
    );

    console.log("Copy the private key into hardhat.config to be used!");
    console.log(chalk.white.bgRed.bold(deployer.privateKey));
    console.log(
        chalk.bold("After you enter a password phrase will be deleted !")
    );
...
task("get-private-key", "Get private key of account")
    ...
    .setAction(async (args, hre) => {
        ...
        let wallet = hre.ethers.Wallet.fromEncryptedJsonSync(
            wallet_json,
            password
        );
        ...
        let pressEnter = prompt(
            "Did you save it the private key (press Enter to continue and delete logs)    "
        );
        clearLines(3);
...
task("fund-deployer", "Fund the deployer account with native tokens")
    ...
    .setAction(async (args, hre) => {
        try {
            let networks = hre.config.superchain.networks;
            let deployer = new hre.ethers.Wallet(
                hre.config.superchain.deployerAccount
            );
            let funder = new hre.ethers.Wallet(
                hre.config.superchain.funderAccount
            );
            ...
            let data = (await contract.getDeployTransaction()).data;
            ...
            for (let i = 0; i < networks.length; i++) {
                console.log(
                    chalk.bold("-->Checking for balances on network "),
                    chalk.bold.green(networks[i])
                );

                let provider = new hre.ethers.JsonRpcProvider(
                    hre.config.networks[networks[i]].url
                );
                console.log("\n");
                console.log("Connected to proivder");
                let deployerBalance = hre.ethers.formatEther(
                    await provider.getBalance(deployer.address)
                );
                let funderBalance = hre.ethers.formatEther(
                    await provider.getBalance(funder.address)
                );
                ...
                let fundToSend = hre.ethers.formatEther(
                    (gas * BigInt(estimatedGas) * BigInt(2)).toString()
                );

                console.log(
                    chalk.bold(
                        "Required amount of native tokens to deploy contract:  "
                    ),
                    chalk.bold.green(fundToSend)
                );

                if (deployerBalance >= fundToSend) {
                    console.log(
                        "Balance for deployer address: ",
                        chalk.bold(deployer.address),
                        " ",
                        chalk.bold.green(deployerBalance)
                    );

                    console.log(
                        chalk.bold.green(
                            "Deployer has enough amount of tokens passing to next network\n"
                        )
                    );
                } else {
                    console.log(
                        ...
                        chalk.bold(deployer.address),
                        " ",
                        chalk.bold.red(deployerBalance)
                    );

                    console.log(
                        "Balance for funder address:  ",
                        chalk.bold(funder.address),
                        " ",
                        chalk.bold.green(funderBalance),
                        "\n"
                    );

                    let ifFund = prompt(
                        "Fund the deployer address (press Y and enter to continue)  "
                    );
                    ...
                    if (ifFund.toLowerCase() === "y") {
                        ...
                        console.log(
                            chalk.bold.green(
                                "Tx sent for funding the deployer passing to next network\n"
                            )
                        );
                    }
                }
            ...
            console.log(
                chalk.bold.green(
                    "Funding phase is completed continue with deploying"
                )
            );
        } catch (err) {
            console.log(err);
...
task(
    "check-balances",
    "Check balances of deployer and funder on wanted chains"
).setAction(async (args, hre) => {
    try {
        let networks = hre.config.superchain.networks;
        let deployer = new hre.ethers.Wallet(
        ...
        for (let i = 0; i < networks.length; i++) {
            ...
            console.log(
                ...
                chalk.bold(deployer.address),
                " ",
                chalk.bold.blue(deployerBalance)
            );
            ...
            if (deployerBalance >= fundToSend) {
                console.log(
                    chalk.bold.green("Deployer has enough amount of funds")
                );
            } else {
                console.log(
                    chalk.bold.red(
                        "Deployer does not have enough amount of tokens, fund it!\n\n"
                    )
                );
                funded = false;
            }
...
task("superchain-deploy", "Deploy smart contracts to superchain")
    ...
    .setAction(async (args, hre) => {
        ...
        for (let i = 0; i < networks.length; i++) {
            ...
            console.log("\n\n");
            console.log(
                chalk.bold.green("Deployed successfully to ", networks[i])
            );
...
task("superchain-verify", "Verify contracts on deployed networks")
    ...
    .setAction(async (args, hre) => {
        ...
        for (let i = 0; i < networks.length; i++) {
            ...
            let res = execSync(
                `npx hardhat verify --network ${networks[i]} ${args.address} ${args.constructorArgs}`
            );
...
module.exports = {
    ...
    networks: {
        hardhat: {},
        sepolia: {
            url: "https://sepolia.infura.io/v3/48e285e36f744446995bf5b78ed2a8da",
        },
        fuji: {
